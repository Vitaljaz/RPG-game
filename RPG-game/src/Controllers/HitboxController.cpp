...
HitboxController::HitboxController(sf::Sprite& sprite, float offset_x, float offset_y, float width, float height) :
...
bool HitboxController::checkCollision(const sf::FloatRect& frect)
{
	return hitbox.getGlobalBounds().intersects(frect);
}

void HitboxController::update()
{
